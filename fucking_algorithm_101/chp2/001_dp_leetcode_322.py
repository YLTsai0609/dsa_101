...
def coin_change_recursion_with_cache(coins : List[int], amount : int) -> int:
    ...
    def dp(coins : List[int], amount : int) -> int:
        ...
        # 如果在 cache 裡面，直接 return
        if cached[amount] != float('inf'):
...
# bottom-up
def coin_change_iteration_with_cached(coins : List[int],
                                      amount : int,
...
for coins, amount, n_coin_count in questions:
    ...
    for _func in [
        ...
        assert pred == n_coin_count, f'expected {n_coin_count}, got : {pred}, func : {_func.__name__}'
