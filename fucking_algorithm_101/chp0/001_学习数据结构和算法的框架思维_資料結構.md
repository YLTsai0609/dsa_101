...
1. 靜態數組目前只存在在 C++, Java, Golang， Python, JS 沒有提供
2. 靜態術組是連續記憶體，指針指向頭
   1. int arr[10]
   2. memset(arr, 0, sizeof(arr))
   3. arr[0] = 1
   4. arr[1] = 2
   5. arr[0] <-- access the value
   6. access time complexity = O(1)
以上是 CRUD(Create, Read, Update, Delete) 的 C, R

## 增 (Append)

- 結尾 -- append - O(1)，直接 access arr[5] = val
- 插入? -- insert - O(N)，涉及資料搬移 arr[2] = val, 3, 4, 5 要往後搬移
- 把靜態 array 變大 -- O(N) --> 擴容操作

## 刪(Delete)
- 結尾 -- arr[10] = null or arr[10] = -1 - O(1)
- 插入? -- insert - O(N)，涉及資料搬移 arr[2] = null, 3, 4, 5 要往"前"搬移

## 擴容

擴充容量不會每次發生，如果一次請求大一點的 memory size，可以確保 append 都是 O(1)

# [動態數組](https://labuladong.online/algo/data-structure-basic/array-implement/#%E5%8A%A8%E6%80%81%E6%95%B0%E7%BB%84%E4%BB%A3%E7%A0%81%E5%AE%9E%E7%8E%B0)

* 如果數組快滿了，就做擴容，所以 append 仍然是 O(1)，但有時候會產生擴容量
* **只有給定 index， Access 才是 O(1)，如果是給 value，要找 index，那會是 O(N)，除非 array 是 sorted ，那麼會是 O(logN)**
* 如果 element 超過 1/2，那就擴充容量，小於 1/2，那就縮容
